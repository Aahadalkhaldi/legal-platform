...
export async function POST(
  ...
  try {
    ...
    await assertMatterActionAccess(supabase, context, matterId, "create_appeal");
    ...
    if (existingTransition) {
      throw new ApiError("CONFLICT", "Appeal proceeding already exists for this source proceeding.");
    }
    ...
    const insertPayload = buildProceedingTransitionInsert({
      ...
      targetStage: payload.stage ?? "appeal",
    ...
    await writeAuditEvent({
      context,
      action: "MATTER_PROCEEDING_APPEAL_CREATED",
